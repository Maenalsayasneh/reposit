package io.reactivex.internal.schedulers;

import java.util.concurrent.Callable;
import java.util.concurrent.Future;
import java.util.concurrent.atomic.AtomicReferenceArray;
import k0.b.w.a;

public final class ScheduledRunnable extends AtomicReferenceArray<Object> implements Runnable, Callable<Object>, a {
    public static final Object c = new Object();
    public static final Object d = new Object();
    public static final Object q = new Object();
    public static final Object x = new Object();
    public final Runnable y;

    public ScheduledRunnable(Runnable runnable, k0.b.z.a.a aVar) {
        super(3);
        this.y = runnable;
        lazySet(0, aVar);
    }

    public void a(Future<?> future) {
        Object obj;
        do {
            obj = get(1);
            if (obj != x) {
                if (obj == d) {
                    future.cancel(false);
                    return;
                } else if (obj == q) {
                    future.cancel(true);
                    return;
                }
            } else {
                return;
            }
        } while (!compareAndSet(1, obj, future));
    }

    public Object call() {
        run();
        return null;
    }

    /* JADX WARNING: Removed duplicated region for block: B:0:0x0000 A[LOOP_START, MTH_ENTER_BLOCK] */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public void dispose() {
        /*
            r7 = this;
        L_0x0000:
            r0 = 1
            java.lang.Object r1 = r7.get(r0)
            java.lang.Object r2 = x
            r3 = 0
            if (r1 == r2) goto L_0x0031
            java.lang.Object r2 = d
            if (r1 == r2) goto L_0x0031
            java.lang.Object r4 = q
            if (r1 != r4) goto L_0x0013
            goto L_0x0031
        L_0x0013:
            r5 = 2
            java.lang.Object r5 = r7.get(r5)
            java.lang.Thread r6 = java.lang.Thread.currentThread()
            if (r5 == r6) goto L_0x0020
            r5 = r0
            goto L_0x0021
        L_0x0020:
            r5 = r3
        L_0x0021:
            if (r5 == 0) goto L_0x0024
            r2 = r4
        L_0x0024:
            boolean r0 = r7.compareAndSet(r0, r1, r2)
            if (r0 == 0) goto L_0x0000
            if (r1 == 0) goto L_0x0031
            java.util.concurrent.Future r1 = (java.util.concurrent.Future) r1
            r1.cancel(r5)
        L_0x0031:
            java.lang.Object r0 = r7.get(r3)
            java.lang.Object r1 = x
            if (r0 == r1) goto L_0x004b
            java.lang.Object r1 = c
            if (r0 == r1) goto L_0x004b
            if (r0 != 0) goto L_0x0040
            goto L_0x004b
        L_0x0040:
            boolean r1 = r7.compareAndSet(r3, r0, r1)
            if (r1 == 0) goto L_0x0031
            k0.b.z.a.a r0 = (k0.b.z.a.a) r0
            r0.delete(r7)
        L_0x004b:
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: io.reactivex.internal.schedulers.ScheduledRunnable.dispose():void");
    }

    public boolean isDisposed() {
        Object obj = get(0);
        if (obj == c || obj == x) {
            return true;
        }
        return false;
    }

    public void run() {
        Object obj;
        Object obj2;
        lazySet(2, Thread.currentThread());
        try {
            this.y.run();
        } catch (Throwable th) {
            lazySet(2, (Object) null);
            Object obj3 = get(0);
            if (!(obj3 == c || !compareAndSet(0, obj3, x) || obj3 == null)) {
                ((k0.b.z.a.a) obj3).delete(this);
            }
            do {
                obj2 = get(1);
                if (obj2 == d || obj2 == q || compareAndSet(1, obj2, x)) {
                    throw th;
                }
                obj2 = get(1);
                break;
            } while (compareAndSet(1, obj2, x));
            throw th;
        }
        lazySet(2, (Object) null);
        Object obj4 = get(0);
        if (!(obj4 == c || !compareAndSet(0, obj4, x) || obj4 == null)) {
            ((k0.b.z.a.a) obj4).delete(this);
        }
        do {
            obj = get(1);
            if (obj == d || obj == q || compareAndSet(1, obj, x)) {
            }
            obj = get(1);
            return;
        } while (compareAndSet(1, obj, x));
    }
}
