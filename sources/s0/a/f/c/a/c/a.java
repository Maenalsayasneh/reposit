...
public class a extends KeyFactorySpi implements AsymmetricKeyInfoConverter {
    public PrivateKey engineGeneratePrivate(KeySpec keySpec) throws InvalidKeySpecException {
        if (keySpec instanceof PKCS8EncodedKeySpec) {
            try {
                return new BCNHPrivateKey(p.s(r.x(((PKCS8EncodedKeySpec) keySpec).getEncoded())));
            } catch (Exception e) {
    ...
    public PublicKey engineGeneratePublic(KeySpec keySpec) throws InvalidKeySpecException {
        if (keySpec instanceof X509EncodedKeySpec) {
            try {
                return new BCNHPublicKey(m0.s(((X509EncodedKeySpec) keySpec).getEncoded()));
            } catch (Exception e) {
                throw new InvalidKeySpecException(e.toString());
            }
        } else {
    ...
    public PublicKey generatePublic(m0 m0Var) throws IOException {
        return new BCNHPublicKey(m0Var);
    }
}
